...
encode `sex`, `apoe4`, and `age_at_onset`. A total of 479 participants
were recruited, and 93 of them developed MCI. The average baseline age
is 70.4064516. The proportion of women who are APOE4 carriers in the
study is 0.4384134.
...
There are 8 observations that appear in only the baseline dataset and16
observations that appear in only the amyloid dataset.In order to combine
these two datasets, we need to remove those which only appear in 1
dataset.
...
are retained. It has 471 observations and `rcol(mci)` variables. `NA`
exists in the `age_at_onset` to mean the participant remains MCI free
during the follow-up period. Some `NA` values exist in the `time_n`
variables, indicating that the Amyloid β 42/40 ratio is missing for the
corresponding participant at that visit. The exported file is `mci.csv`.
