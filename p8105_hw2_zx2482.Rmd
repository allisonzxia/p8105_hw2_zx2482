...
There are `r nrow(mci_baseline_only)` observations that appear in only the baseline dataset and`r nrow(mci_amyloid_only)` observations that appear in only the amyloid dataset.In order to combine these two datasets, we need to remove those which only appear in 1 dataset.
