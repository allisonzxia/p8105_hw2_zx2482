...
str(data_trash)
```
The `mtw` data has `r nrow(mtw)` observations and `r ncol(mtw)` variables, the `ptw` data has `r nrow(ptw)` observations and `r ncol(ptw)` and `gtw` data has `r nrow(gtw)` observations and `r ncol(gtw)`.The datasets tell us about litters that the Mr. Trash Wheel, Professor Trash Wheel, and the Gwynnda Trash Wheel has removed as a water wheel vessel from the Inner Harbor in Baltimore, Maryland from years `r range(gtw$year,mtw$year,ptw$year)[1]` to `r range(gtw$year,mtw$year,ptw$year)[2]`. The total weight of trash collected by Professor Trash Wheel is `r filter(data_trash, wheel == "ptw") |> pull(weight_tons) |> sum() |> round(2)` tons. In July of 2021, the total number of cigarette butts collected by Gwynnda is `r filter(data_trash, wheel == "gtw", year == 2021, month_num == 7) |> pull(cigarette_butts) |> sum () |> round(2)` 
