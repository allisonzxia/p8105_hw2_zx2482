...
There are `r nrow(mci_baseline_only)` observations that appear in only the baseline dataset and `r length(unique(mci_amyloid_only$id))` observations that appear in only the amyloid dataset.In order to combine these two datasets, we need to remove those which only appear in 1 dataset.
...
```
The dataset `mci` is the combined dataset of both the demographic and biomarker datasets so that only participants who appear in both datasets are retained. It has `r nrow(mci)` observations and `rcol(mci)` variables. `NA` exists in the `age_at_onset` to mean the participant remains MCI free during the follow-up period. Some `NA` values exist in the `time_n` variables, indicating that the Amyloid β 42/40 ratio is missing for the corresponding participant at that visit. The exported file is `mci.csv`.
