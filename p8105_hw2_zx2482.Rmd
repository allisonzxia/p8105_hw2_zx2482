...
str(mci_baseline)
```
I use the parameter `skip = 1` when importing `MCI_baseline.csv` to skip the first row, which is not the column names, to ensure that the file is read correctly. Then, I use `janitor::clean_names()` to clean all variable names. I use `filter()` to remove any participants who do not have MCI during the follow-up period. Lastly, I use `mutate()`, `case_when()`, `as.character()`, and `as.numeric()` to appropriately encode `sex`, `apoe4`, and `age_at_onset`. A total of `r nrow(mci_baseline_original)` participants were recruited, and `r nrow(mci_baseline)` of them developed MCI. The average baseline age is `r mean(mci_baseline$age_at_onset)`. The proportion of women who are APOE4 carriers in the study is `r nrow(filter(mci_baseline, sex == "f"))/nrow(mci_baseline)`.
